...
Graph memory represents knowledge as interconnected nodes and relationships, similar to how humans form mental models. This type of memory offers:

- **Relationship-based knowledge**: Understanding how entities relate to each other
- **Contextual retrieval**: Finding information through relationship traversals
- **Flexible querying**: Connecting topics in ways not possible with vector searches alone

### Available Implementations

Mem0 provides two graph memory implementations:

| Implementation | Type | Benefits | Use Case |
|----------------|------|----------|----------|
| Neo4j | External database | Powerful query capabilities, scalable for large datasets | Production deployments with complex graph needs |
| Kuzu | Embedded database | Simplified deployment, combined vector+graph capabilities | Lightweight applications, development environments |

The Kuzu implementation stores both entity nodes and their relationships in a single embedded database, eliminating the need for external dependencies while providing seamless integration with vector-based memory.